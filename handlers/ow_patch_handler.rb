...
class OwPatchHandler < CommandHandler
  ...
  command(:patches, :show_live_patches)
    .feature(:owpatch).no_args.usage('patches')
  ...
  def search_patch_notes(event, *search_terms)
    handle_errors(event) do
      ...
      patch_notes_response = patch_notes_client.get_patch_notes('overwatch')
      ...
      event.channel.send_embed(' ') do |embed|
        basic_embed(embed)
        embed.author = { name: 'Overwatch Patch Notes Search', url: "#{config.base_url}/patch_notes/overwatch" }
        embed.description = "Results for query: #{search_terms.join(' ')}"

        patch_notes_response.patches.each do |patch|
          results = patch.search(search_terms.join(' '))
          next if results.empty?

          embed.add_field(name: patch.title, value: results.map(&:format_str).join("\n"))
